<?php
	function _publish($id){
		if (is_array($id)){
			if ($id[1] == 'null'){return array(array('call', 'message', "Ќе выбран не один элемент"));}
			else{
				$errors = array();
				$to = ($id[0] == 'true') ? '1' : '0';
				$mes = ($id[0] == 'false') ? "Ёлементы заблокированы" : "Ёлементы разблокированы";
				foreach ($id[1] as $ids){
				...
				else return array(array('call', 'message', "ќшибка обработки ID: ".implode(', ', $errors)));
			}
		}else{
