...
'User title help'   			 => 'Это название перекроет ранги пользователям, включенных в группу. Оставьте пустым, что бы не менять ранги.',    //This title will override any rank users in this group have attained. Leave blank to use default title or rank.
...
'Allow view users label'		 => 'Разрешить просмотр списка пользователей и их профайлов.',    //Allow users to view the user list and user profiles.
...
'Allow moderate help'   		 => 'Для того, чтобы пользователь в этой группе имел возможности модератора, он должен быть назначен модератором хотя бы в одном из форумов. Это можно сделать на странице администрирования пользователя, выбрав вкладку Администрирование в Профайле пользователя.',    //In order for a user in this group to have moderator abilities, he/she must be assigned to moderate one or more forums. This is done via the user administration page of the user's profile.
...
'Moderator default group'   	 => 'Это группа «по умолчанию» для новых пользователей и она не должна содержать прав модератора.',    //This is the default group for new users and therefore cannot be assigned moderator privileges.
